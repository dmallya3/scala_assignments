...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def max(elem: Tweet, left: Tweet, right: Tweet): Tweet = {
    if( elem.retweets > left.retweets ) {
      if( elem.retweets > right.retweets )  elem
      else right
    } else {
  ...
  def check(x: TweetSet): Tweet = {
    if( x.isEmpty )  new Tweet(null, null, 0)
    else x.mostRetweeted
  }

  def mostRetweeted: Tweet = {
//    if( this.isEmpty ) new Tweet(null, null, 0)
//    else max(elem, left.mostRetweeted, right.mostRetweeted)
    max(elem, check(left), check(right))
  }
